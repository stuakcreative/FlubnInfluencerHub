import { getBrandPlan } from "../../utils/brandSubscription";
...
import {
  ...
  Inbox, Lock, ChevronDown, Briefcase,
} from "lucide-react";
...
// Deterministic campaign colour from campaign name
...
export default function BrandChats() {
  ...
  const { user } = useAuth();
  // Only show this brand's own conversations
  const requests = allRequests.filter((r) => r.brandId === user?.id);
  const [searchParams, setSearchParams] = useSearchParams();
  const navigate = useNavigate();

  const [search, setSearch] = useState("");
  const [brandPlan] = useState(() => getBrandPlan());

  const activeId = searchParams.get("id");
  const [mobileView, setMobileView] = useState<"list" | "chat">(activeId ? "chat" : "list");

  // Which groups are collapsed { influencerId → true }
  ...
  const userId = user?.id ?? "brand_user";

  // All accepted conversations, newest activity first
  ...
  const filtered = conversations.filter(
    (r) =>
      r.influencerName?.toLowerCase().includes(search.toLowerCase()) ||
      r.campaignName.toLowerCase().includes(search.toLowerCase())
  );

  // Group by influencerId (fallback to name)
  const groups = (() => {
    const map = new Map<string, typeof filtered>();
    for (const conv of filtered) {
      const key = conv.influencerId ?? conv.influencerName ?? "unknown";
    ...
    return Array.from(map.entries())
      .map(([key, convs]) => {
        ...
        const totalUnread = convs.reduce(
          (sum, c) => sum + getUnreadCount(userId, c.id, c.chatMessages ?? [], "brand"),
          0
        );
  ...
  const totalUnread = conversations.reduce(
    (sum, r) => sum + getUnreadCount(userId, r.id, r.chatMessages ?? [], "brand"),
    0
  );
          ...
          {groups.length === 0 ? (
              ...
              {conversations.length === 0 ? (
                <>
                  ...
                  <p className="text-[#94a3b8] text-xs mt-1 max-w-[200px]">
                    Chats unlock when an influencer accepts your request
                  </p>
                  <button
            ...
            <div className="py-2">
              {groups.map(({ key, convs, totalUnread: groupUnread }) => {
                const rep = convs[0]; // representative for avatar/name
                const isMulti = convs.length > 1;
                const isGroupCollapsed = !!collapsed[key];

                if (!isMulti) {
                  // ── Single conversation: flat row ──────────────────────
                  const req = convs[0];
                  const isActive = req.id === activeId;
                  const unread = getUnreadCount(userId, req.id, req.chatMessages ?? [], "brand");
                  ...
                  return (
                    ...
                    >
                      <div className="relative shrink-0">
                        <ProfileAvatar
                          photo={req.influencerPhoto}
                          name={req.influencerName}
                          size="w-11 h-11"
                          userId={req.influencerId}
                          userType="influencer"
                          requestId={req.id}
                          clickable={false}
                        />
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center justify-between gap-2 mb-0.5">
                          <div className="flex items-center gap-1 min-w-0">
                            <span className="text-sm text-[#1a1a2e] truncate">{req.influencerName}</span>
                            {req.influencerId && isInfluencerVerified(req.influencerId) && <VerifiedBadge size={12} />}
                          </div>
                ...
                return (
                  <div key={key}>
                    ...
                    >
                      <div className="relative shrink-0">
                        <ProfileAvatar
                          photo={rep.influencerPhoto}
                          name={rep.influencerName}
                          size="w-11 h-11"
                          userId={rep.influencerId}
                          userType="influencer"
                          requestId={rep.id}
                          clickable={false}
                        />
                        {/* Project count badge on avatar */}
                        <span
                          className="absolute -bottom-0.5 -right-0.5 w-5 h-5 rounded-full bg-[#2F6BFF] text-white text-[9px] flex items-center justify-center border-2 border-white"
                        >
                          {convs.length}
                        </span>
                      </div>
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-1.5 min-w-0">
                          <span className="text-sm text-[#1a1a2e] truncate">{rep.influencerName}</span>
                          {rep.influencerId && isInfluencerVerified(rep.influencerId) && <VerifiedBadge size={12} />}
                        </div>
        ...
        <div className="md:hidden flex items-center gap-2 px-4 py-3 bg-white border-b border-[#e2e8f0] shrink-0">
          ...
          {activeRequest && (
            <div className="min-w-0">
              <span className="text-[#1a1a2e] text-sm block truncate">{activeRequest.influencerName}</span>
            ...
            {activeId && brandPlan.isPaid ? (
              ...
              >
                <ChatPanel
                  ...
                  currentUserName={user?.name ?? "Brand"}
                  embedded={true}
                />
              </motion.div>
            ) : activeId && !brandPlan.isPaid ? (
              ...
              >
                ...
                <h2 className="text-[#1a1a2e] text-xl mb-2">Chat is a Paid Feature</h2>
                <p className="text-[#64748b] text-sm max-w-[300px] mb-6">
                  Upgrade your plan to message influencers directly, negotiate budgets, and manage campaigns in real time.
                </p>
                <a
                  href="/brand/subscription"
                  className="inline-flex items-center gap-2 px-6 py-3 rounded-2xl text-white text-sm hover:opacity-90 transition-all"
              ...
              >
                ...
                <p className="text-[#64748b] text-sm max-w-[280px]">
                  Select a conversation from the left to start chatting with an influencer.
                </p>

                {conversations.length > 0 && (
                  <div className="mt-6 flex flex-col gap-2 w-full max-w-[300px]">
                    {conversations.slice(0, 3).map((r) => {
                      const unread = getUnreadCount(userId, r.id, r.chatMessages ?? [], "brand");
                      ...
                      return (
                        ...
                        >
                          <ProfileAvatar
                            photo={r.influencerPhoto}
                            name={r.influencerName}
                            size="w-8 h-8"
                            textSize="text-xs"
                          />
                          <div className="flex-1 min-w-0">
                            <p className="text-sm text-[#1a1a2e] truncate">{r.influencerName}</p>
                            <p
